...
export const getMembersWithSepaMandate = query({
  ...
  handler: async (ctx, { vereinId }) => {
    ...
    const membersWithBeitragssatz = members.filter(
      (m) => m.beitragsSatzId !== undefined,
    );

    // Get beitragssatz for each member
    const membersWithDetails = await Promise.all(
      membersWithBeitragssatz.map(async (member) => {
        const beitragssatz = member.beitragsSatzId
          ? await ctx.db.get("beitrags_satz", member.beitragsSatzId)
          : null;
        return {
          ...member,
          beitragssatz,
