...
async function ensureRoleIdsBelongToVerein(
  ctx: MutationCtx,
  vereinId: Id<"verein">,
  rollen: readonly Id<"vereins_rollen">[],
) {
...
async function countMitgliederForVerein(
  ctx: MutationCtx,
  vereinId: Id<"verein">,
) {
...
export const search = query({
  ...
  async handler(ctx, { vereinId, searchTerm }) {
    ...
    const mitglieder = await ctx.db
      .query("mitglied")
      .withSearchIndex("mitglied_search", (q) =>
        q.search("searchName", searchTerm).eq("vereinId", vereinId),
      )
      .take(20);
    return mitglieder;
  },
...
export const create = mutation({
  args: {
    ...
    typ: v.union(
      v.literal("bewerber"),
      v.literal("mitglied"),
      v.literal("fördermitglied"),
      v.literal("kontakt"),
      v.literal("ausgeschieden"),
    ),
    geschlecht: v.union(
      v.literal("m"),
      v.literal("w"),
    ...
    beitragsEinzug: v.optional(
      v.union(v.literal("r"), v.literal("l"), v.literal("b"), v.literal("p")),
    ),
  ...
  handler: async (ctx, values) => {
    ...
    if (values.rollen && values.rollen.length > 0) {
      ...
      await ensureRoleIdsBelongToVerein(
        ctx as MutationCtx,
        values.vereinId,
        values.rollen,
      );
...
export const remove = mutation({
  ...
  handler: async (ctx, { id }) => {
    ...
    await Promise.all(
      listenEintraege.map((eintrag) => ctx.db.delete(eintrag._id)),
    );
  ...
  handler: async (ctx, { mitgliedId, email }) => {
    ...
    await Promise.all(
      existingInvites
        ...
        .map((invite) => ctx.db.patch(invite._id, { usedAt })),
    );
    ...
    await ctx.scheduler.runAfter(
      0,
      internal.sendMails.sendMitgliedLinkInviteEmail,
      {
        to: email,
        link,
