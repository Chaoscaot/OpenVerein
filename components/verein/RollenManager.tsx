...
export function RollenManager({ vereinId }: { vereinId: Id<"verein"> }) {
  ...
  const editRole = useMemo(
    () => rollen?.find((rolle) => rolle._id === editId),
    [rollen, editId],
  );
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between gap-4">
        <div>
          <CardTitle>Rollen & Berechtigungen</CardTitle>
          <CardDescription>
            Erstelle Rollen und bündele feingranulare Rechte für
            Vereinsbereiche.
          </CardDescription>
        </div>
        {canManage && (
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="max-h-[85vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>
                  {editId ? "Rolle bearbeiten" : "Neue Rolle"}
                </DialogTitle>
                <DialogDescription>
                  Wähle die Rechte aus, die diese Rolle erhalten soll.
                </DialogDescription>
              </DialogHeader>
          ...
          (rollen ?? []).map((rolle) => (
            <div key={rolle._id} className="rounded-md border p-3">
                ...
                {canManage && (
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      variant="destructive"
                      size="sm"
                      onClick={() => handleDelete(rolle._id)}
                    >
                      Löschen
                    </Button>
                  </div>
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                {rolle.berechtigungen.length} Rechte
              </p>
            </div>
          ))
        )}
        {editRole && open ? (
          <div className="sr-only">Bearbeite {editRole.name}</div>
        ) : null}
      </CardContent>
    </Card>
  );
