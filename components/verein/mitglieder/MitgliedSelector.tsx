...
export function MitgliedSelector({
  value,
  vereinId,
  onChange,
}: {
  value?: Id<"mitglied"> | undefined;
  vereinId: Id<"verein">;
  onChange?: (id: Id<"mitglied"> | undefined) => void;
}) {
  const [search, setSearch] = useState("");
  const debouncedSearch = useDebounce(search, 300);
  const mitglieder = useQuery(api.mitglieder.search, {
            ...
            <CommandGroup>
              {mitglieder?.map((mitglied) => (
                ...
                >
                  <HugeiconsIcon
                    icon={Check}
                    className={`size-4 shrink-0 ${value === mitglied._id ? "opacity-100" : "opacity-0"}`}
                  />
