...
const formSchema = z
  .object({
    ...
    postalCode: z
      .string("Bitte gib eine Postleitzahl ein")
    ...
    if (data.austrittsdatum) {
      if (data.austrittsdatum < data.beitrittsdatum) {
        ctx.addIssue({
          code: "custom",
          message:
            "Das Austrittsdatum darf nicht vor dem Beitrittsdatum liegen",
          path: ["austrittsdatum"],
        });
      }
      if (data.typ === "mitglied" || data.typ === "fördermitglied") {
        ctx.addIssue({
          code: "custom",
          message:
            "Nur Bewerber, Kontakte und ausgeschiedene Mitglieder können ein Austrittsdatum haben",
        });
      }
    }
  });

export function MitgliederEdit({
  verein,
  mitglied,
}: {
  verein: Doc<"verein">;
  ...
  const canLinkAccount =
    acl?.permissions.includes("mitglied.linkAccount") ?? false;
  const canAssignRoles = acl?.permissions.includes("rolle.assign") ?? false;

  const vereinsRollen = useQuery(
    api.rollen.list,
    canAssignRoles ? { vereinId: verein._id } : "skip",
  );
  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      geburtsdatum: mitglied?.geburtsdatum
        ? new Date(mitglied.geburtsdatum)
        : undefined,
      beitrittsdatum: mitglied?.beitrittsdatum
        ? new Date(mitglied.beitrittsdatum)
    ...
    onSubmit: async (values) => {
      ...
      if (mitglied) {
        await updateUser({
          ...
          geburtsdatum: values.value.geburtsdatum
            ? values.value.geburtsdatum.toISOString().split("T")[0]
            : undefined,
          austrittsdatum: values.value.austrittsdatum
            ? values.value.austrittsdatum.toISOString().split("T")[0]
            : undefined,
          beitrittsdatum: values.value.beitrittsdatum
            .toISOString()
            .split("T")[0],
          familienstand:
            values.value.familienstand && values.value.familienstand.length > 0
              ? values.value.familienstand
              : undefined,
          parent: values.value.parent as Id<"mitglied"> | undefined,
          beitragsSatzId: values.value.beitragsSatzId as
            | Id<"beitrags_satz">
            | undefined,
          datein: values.value.datein.map((d) => ({ name: d.name, id: d.id })),
          rollen: canAssignRoles
            ? (values.value.rollen?.map((id) => id as Id<"vereins_rollen">) ??
              [])
            : undefined,
          sepaMandat:
            values.value.sepaIban &&
            values.value.sepaBic &&
            values.value.sepaMandatErstelltAm
              ? {
                  iban: values.value.sepaIban,
                  bic: values.value.sepaBic,
                  erstelltAm: values.value.sepaMandatErstelltAm
                    .toISOString()
                    .split("T")[0],
                }
              : undefined,
  ...
  const [ehrenmitglied, setEhrenmitglied] = useState(
    mitglied?.ehrenmitglied ?? false,
  );
  const countries = useMemo(() => getCountries(), []);
  const [uploading, setUploading] = useState(false);
  const deleteFile = useMutation(api.files.deleteFile);
  const requestAccountLinkInvite = useMutation(
    api.mitglieder.requestAccountLinkInvite,
  );
  ...
  return (
    ...
    >
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectGroup>
                          ...
                          <SelectItem value="fördermitglied">
                            Fördermitglied
                          </SelectItem>
                          <SelectItem value="kontakt">Kontakt</SelectItem>
                          <SelectItem value="ausgeschieden">
                            Ausgeschieden
                          </SelectItem>
                        </SelectGroup>
                      </SelectContent>
                    ...
                    return (
                      <Field data-invalid={isInvalid}>
                        ...
                        <Select
                          value={field.state.value}
                          onValueChange={(v) =>
                            field.handleChange(v as "m" | "w" | "d" | "n")
                          }
                        >
                          <SelectTrigger>
                            <SelectValue />
                        ...
                        >
                          ...
                          <SelectContent>
                            <SelectGroup>
                              ...
                              <SelectItem value="verheiratet">
                                Verheiratet
                              </SelectItem>
                              <SelectItem value="geschieden">
                                Geschieden
                              </SelectItem>
                              <SelectItem value="verwitwet">
                                Verwitwet
                              </SelectItem>
                            </SelectGroup>
                          </SelectContent>
      ...
      <SimpleCard
        title="Kontakt"
        description="Kontaktinformationen des Mitglieds"
      >
        <FieldGroup>
          <form.Field
            name="email"
              ...
              return (
                <Field data-invalid={isInvalid} orientation="responsive">
                  <div className="space-y-1">
                    <FieldLabel htmlFor={field.name}>
                      Vereins-Rundmails erhalten
                    </FieldLabel>
                    <p className="text-sm text-muted-foreground">
                      Wenn deaktiviert, wird die Person bei Listenmails
      ...
      <SimpleCard
        ...
        description="Mitgliedschaft im Verein"
      >
        <FieldGroup>
          <form.Field
            name="beitrittsdatum"
              ...
              return (
                <Field data-invalid={isInvalid}>
                  ...
                  <DatePicker
                    value={field.state.value}
                    onChange={(v) =>
                      v !== undefined ? field.handleChange(v) : null
                    }
                  />
                  {isInvalid && <FieldError errors={field.state.meta.errors} />}
                </Field>
          ...
          <form.Field
            ...
            children={(field) => {
              ...
              return (
                <Field data-invalid={isInvalid}>
                  <FieldLabel htmlFor={field.name}>
                    Übergeordnetes Mitglied
                  </FieldLabel>
                  <MitgliedSelector
                    vereinId={verein._id}
                    value={field.state.value as Id<"mitglied">}
                    onChange={field.handleChange}
                  />
                  {isInvalid && <FieldError errors={field.state.meta.errors} />}
                </Field>
      ...
      <SimpleCard
        ...
        description="Abrechnungsinformationen des Mitglieds"
      >
        <FieldGroup>
          <form.Field
            name="beitragsEinzug"
            listeners={{
              onChange: (v) => {
                setIsSepa(v.value === "l");
                if (!isSepa) {
                  form.setFieldValue("sepaIban", undefined);
                  form.setFieldValue("sepaBic", undefined);
                  form.setFieldValue("sepaMandatErstelltAm", undefined);
                }
                setEhrenmitglied(v.value === undefined);
                if (ehrenmitglied) {
                  form.setFieldValue("sepaIban", "");
                  form.setFieldValue("sepaBic", "");
                  form.setFieldValue("sepaMandatErstelltAm", undefined);
                  form.setFieldValue("beitragsSatzId", undefined);
                }
              },
            }}
            children={(field) => {
            ...
            children={(field) => {
              ...
              return (
                <Field
                  ...
                  className={
                    ehrenmitglied ? "opacity-50 pointer-events-none" : ""
                  }
                >
                  <FieldLabel htmlFor={field.name}>Beitragssatz</FieldLabel>
                  <BeitragssatzSelector
                    verein={verein}
                    value={field.state.value as Id<"beitrags_satz">}
                    onChange={field.handleChange}
                  />
                  {isInvalid && <FieldError errors={field.state.meta.errors} />}
                </Field>
          ...
          <form.Field
            ...
            children={(field) => {
              ...
              return (
                <Field
                  data-invalid={isInvalid}
                  className={isSepa ? "" : "opacity-50 pointer-events-none"}
                >
      ...
      {(canLinkAccount || canAssignRoles) && (
        <SimpleCard
          title="Konto & Rollen"
          description="OpenVerein-Konto verknüpfen und Rollen zuweisen"
        >
          <FieldGroup>
            {canLinkAccount && (
              <Field>
                ...
                <div className="flex items-center gap-3 text-sm">
                  ...
                  >
                    {accountLinked ? "✓ Verknüpft" : "✗ Nicht verknüpft"}
                  </span>
                  {mitglied ? (
                    accountLinked ? (
                      <Button
                      ...
                      >
                        ...
                        <DialogContent>
                          <DialogHeader>
                            ...
                            <DialogDescription>
                              Empfänger eingeben. Der Link führt zur
                              Konto-Anmeldung/Registrierung und danach zur
                              Verknüpfung.
                            </DialogDescription>
                          </DialogHeader>
                          ...
                          <DialogFooter>
                            <Button
                              ...
                              onClick={async () => {
                                ...
                                toast.success(
                                  "Verknüpfungs-E-Mail wurde versendet",
                                );
                              }}
                            >
                              Einladung senden
                    ...
                    <span className="text-muted-foreground">
                      Nach dem Speichern kann eine Verknüpfungs-E-Mail gesendet
                      werden.
                    </span>
                  )}
                </div>
