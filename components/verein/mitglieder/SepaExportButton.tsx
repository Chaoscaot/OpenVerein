...
import { CreditTransfer, CreditTransferPayment } from "better-sepa/dist/types/types";
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            if (!verein.sepa?.iban || !verein.sepa?.bic) {
                toast.error("Bitte hinterlegen Sie die SEPA-Daten des Vereins in den Einstellungen");
                return;
            }
            ...
            const [xml, json] = new SEPA(SEPA_FORMATS["PAIN.001.001.03"]).generateDocument(directDebit);
            ...
            toast.success(`SEPA-XML mit ${payments.length} Lastschriften erfolgreich exportiert`);
        } catch (error) {
