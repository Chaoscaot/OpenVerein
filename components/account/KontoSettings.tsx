...
import { useStoredImageUrl, getStoredImageFileId, toStoredImageRef } from "@/hooks/use-stored-image-url";
...
function setStoredPreferences(preferences: StoredPreferences) {
    ...
    window.localStorage.setItem(ACCOUNT_PREFERENCES_KEY, JSON.stringify(preferences));
...
export function KontoSettings() {
    ...
    const [revokeAllOtherSessionsLoading, setRevokeAllOtherSessionsLoading] = useState(false);
    const [revokingSessionToken, setRevokingSessionToken] = useState<string | null>(null);
    const [avatarFileToDelete, setAvatarFileToDelete] = useState<string | null>(null);
    ...
    const otherSessions = sessions.filter((entry) => entry.token !== currentSessionToken);

    async function loadSessions() {
        setSessionsLoading(true);
        const { data, error } = await authClient.listSessions();
        if (error) {
            toast.error(error.message ?? "Aktive Sitzungen konnten nicht geladen werden.");
    ...
    async function handleAvatarUpload(event: React.ChangeEvent<HTMLInputElement>) {
        ...
        try {
            ...
            if (previousDraftFileId && previousDraftFileId !== persistedFileId && previousDraftFileId !== fileId) {
            ...
            toast.success("Avatar hochgeladen. Speichere dein Profil, um die Änderung zu übernehmen.");
        } catch (error) {
            console.error(error);
    ...
    async function handleEmailSubmit(event: React.FormEvent<HTMLFormElement>) {
        ...
        if (error) {
            setEmailError(error.message ?? "Die E-Mail-Adresse konnte nicht geändert werden.");
            setEmailSaving(false);
            return;
        }

        toast.success("Bitte bestätige die neue E-Mail-Adresse über den Link in deinem Postfach.");
    ...
    async function handlePasswordSubmit(event: React.FormEvent<HTMLFormElement>) {
        ...
        if (error) {
            setPasswordError(error.message ?? "Das Passwort konnte nicht geändert werden.");
    ...
    async function handleSendResetPassword() {
        ...
        if (error) {
            toast.error(error.message ?? "Der Reset-Link konnte nicht versendet werden.");
    ...
    async function handleRevokeOtherSessions() {
        ...
        if (error) {
            toast.error(error.message ?? "Andere Sitzungen konnten nicht beendet werden.");
        ...
        <div className="space-y-6">
                ...
                <CardContent className="flex flex-col gap-4 pt-6 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="space-y-1">
                            <h2 className="text-2xl font-semibold tracking-tight">{sessionData.user.name}</h2>
                            <p className="text-muted-foreground">{sessionData.user.email}</p>
                            <div className="flex flex-wrap gap-2">
                                <Badge variant={sessionData.user.emailVerified ? "default" : "outline"}>{sessionData.user.emailVerified ? "E-Mail bestätigt" : "Bestätigung ausstehend"}</Badge>
                                <Badge variant="outline">{sessions.length} aktive Sitzungen</Badge>
                            </div>
                        </div>
                    </div>
            ...
            <div className="grid gap-6 xl:grid-cols-2">
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Pflege deinen angezeigten Namen und das Profilbild deiner Anmeldung.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <form className="space-y-5" onSubmit={handleProfileSubmit}>
                            <FieldGroup>
                                <Field>
                                    ...
                                    <Input id="account-name" value={currentName} onChange={(event) => setNameDraft(event.target.value)} autoComplete="name" />
                                </Field>
                                <Field>
                                    ...
                                    <div className="flex flex-wrap items-center gap-4">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            <Button type="button" variant="outline" disabled={avatarUploading} onClick={() => document.getElementById("account-avatar-upload")?.click()}>
                                                {avatarUploading ? <Spinner /> : <HugeiconsIcon icon={Upload01Icon} />}
                                                Avatar hochladen
                                            </Button>
                                            {currentImage ? (
                                            ...
                                            <input id="account-avatar-upload" type="file" accept="image/*" className="hidden" onChange={handleAvatarUpload} />
                                        </div>
                                    </div>
                                    <FieldDescription>PNG, JPG oder WebP bis 5MB. Die Änderung wird mit dem Speichern des Profils übernommen.</FieldDescription>
                                </Field>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Lege fest, wie OpenVerein fur dich dargestellt wird und mit dem Browser interagiert.</CardDescription>
                    </CardHeader>
                        ...
                        <FieldGroup>
                            ...
                            <Field orientation="horizontal" className="items-start justify-between gap-4">
                                <div className="space-y-1">
                                    ...
                                    <FieldDescription>
                                        Aktiviert lokale Browser-Benachrichtigungen, sofern dein Browser dies unterstutzt.
                                    </FieldDescription>
                                    {typeof window !== "undefined" && "Notification" in window ? (
                                        <p className="text-muted-foreground text-xs">Browser-Status: {Notification.permission}</p>
                                    ) : null}
                                </div>
                                <Switch checked={browserNotificationsEnabled} onCheckedChange={handleToggleBrowserNotifications} />
                            </Field>
                        </FieldGroup>
                    </CardContent>
                </Card>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Anderungen an der E-Mail mussen bestatigt werden, bevor sie aktiv werden.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <form className="space-y-5" onSubmit={handleEmailSubmit}>
                            <FieldGroup>
                                <Field>
                                    <FieldLabel htmlFor="current-email">Aktuelle E-Mail</FieldLabel>
                                    <Input id="current-email" value={currentEmail} readOnly disabled />
                                </Field>
                                <Field>
                                    <FieldLabel htmlFor="new-email">Neue E-Mail</FieldLabel>
                                    <Input id="new-email" type="email" value={currentNewEmail} onChange={(event) => setNewEmailDraft(event.target.value)} autoComplete="email" />
                                </Field>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Passwort andern und bei Bedarf einen Reset-Link anfordern.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <form className="space-y-5" onSubmit={handlePasswordSubmit}>
                            <FieldGroup>
                                ...
                                <Field>
                                    <FieldLabel htmlFor="confirm-password">Neues Passwort bestatigen</FieldLabel>
                                ...
                                <Field orientation="horizontal" className="items-start justify-between gap-4">
                                    <div className="space-y-1">
                                        ...
                                        <FieldDescription>Beendet nach dem Passwortwechsel alle anderen aktiven Anmeldungen.</FieldDescription>
                                    </div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                ...
                                <Button type="button" variant="outline" disabled={resetPasswordSending} onClick={handleSendResetPassword}>
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Sieh nach, auf welchen Geraten dein Konto aktuell angemeldet ist.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border bg-muted/30 px-4 py-3">
                        <div className="space-y-1">
                            ...
                            <p className="text-muted-foreground text-sm">Beendet alle Sitzungen ausser der gerade verwendeten.</p>
                        </div>
                        <Button type="button" variant="outline" disabled={revokeAllOtherSessionsLoading || otherSessions.length === 0} onClick={handleRevokeOtherSessions}>
                    ...
                    {sessionsLoading ? (
                        ...
                        <div className="overflow-hidden rounded-xl border">
                            {sessions.length === 0 ? (
                                <div className="p-4 text-sm text-muted-foreground">Es wurden keine aktiven Sitzungen gefunden.</div>
                            ) : (
                                            ...
                                            <div className="flex flex-col gap-4 p-4 md:flex-row md:items-center md:justify-between">
                                                <div className="space-y-1">
                                                    <div className="flex flex-wrap items-center gap-2">
                                                        <p className="font-medium">{describeSession(session)}</p>
                                                        {isCurrentSession ? <Badge variant="default">Diese Sitzung</Badge> : null}
                                                    </div>
                                                    <p className="text-muted-foreground text-sm">
                                                        Aktiv seit {formatDate(session.createdAt)} • Gultig bis {formatDate(session.expiresAt)}
                                                    </p>
                                                    <p className="text-muted-foreground text-sm">IP-Adresse: {session.ipAddress ?? "Nicht verfugbar"}</p>
                                                </div>
                                                <Button
                                                    type="button"
                                                    variant="outline"
                                                    disabled={isCurrentSession || revokingSessionToken === session.token}
                                                    onClick={() => handleRevokeSingleSession(session.token)}
                                                >
                                                    {revokingSessionToken === session.token ? <Spinner /> : null}
                                                    Sitzung beenden
                                                </Button>
                                            </div>
                                        </div>
                ...
                <CardFooter className="text-muted-foreground text-xs">
                    Wenn du ein unbekanntes Gerat siehst, andert dein Passwort und melde andere Sitzungen sofort ab.
                </CardFooter>
            </Card>
        </div>
    );
