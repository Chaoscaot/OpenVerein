...
function ActionsCell({ row }: { row: any }) {
  ...
  const handleDelete = async () => {
    if (
      confirm(
        `Möchten Sie das Mitglied ${member.vorname} ${member.nachname} wirklich löschen?`,
      )
    ) {
      try {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        ...
        <DropdownMenuItem
          onClick={() =>
            router.push(`/verein/${member.vereinId}/mitglieder/${member._id}`)
          }
        >
...
export const columns: ColumnDef<Doc<"mitglied">>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <Badge
        variant={row.original.typ === "mitglied" ? "default" : "secondary"}
      >
