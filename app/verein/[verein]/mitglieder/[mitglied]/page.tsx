...
export default async function EditMitgliedPage({
  params,
}: {
  params: Promise<{ verein: string; mitglied: string }>;
}) {
  const { verein, mitglied } = await params;
  const v = await fetchAuthQuery(api.verein.get, {
    id: verein as Id<"verein">,
  });
  const m = await fetchAuthQuery(api.mitglieder.get, {
    id: mitglied as Id<"mitglied">,
  });
