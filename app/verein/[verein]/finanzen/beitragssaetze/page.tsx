...
export default function BeitragssaetzePage() {
    ...
    const [editId, setEditId] = useState<Id<"beitrags_satz"> | undefined>(undefined);
    const [name, setName] = useState("");
    const [betrag, setBetrag] = useState("");
    const [waehrung, setWaehrung] = useState("EUR");
    const [beschreibung, setBeschreibung] = useState("");

    const editBeitragssatz = useMemo(() => beitragssaetze?.find((satz) => satz._id === editId), [beitragssaetze, editId]);
    ...
    const handleSave = async () => {
        ...
        if (!name.trim() || Number.isNaN(parsedBetrag) || parsedBetrag <= 0 || !waehrung.trim()) {
            ...
            <div className="space-y-6 p-4">
                <div className="flex items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-muted-foreground">Verwalte alle Beitragssätze und prüfe die Zuordnung in Buchungen.</p>
                    </div>
                    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                        ...
                        <DialogContent>
                            <DialogHeader>
                                <DialogTitle>{editId ? "Beitragssatz bearbeiten" : "Beitragssatz erstellen"}</DialogTitle>
                                <DialogDescription>Definiere Name, Betrag und optionale Beschreibung.</DialogDescription>
                            </DialogHeader>
                            <div className="grid gap-4 py-4">
                                <div className="grid gap-2">
                                    <Label htmlFor="name">Name</Label>
                                    <Input id="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="z.B. Standardbeitrag" />
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div className="grid gap-2">
                                        ...
                                        <Input id="waehrung" value={waehrung} onChange={(e) => setWaehrung(e.target.value)} placeholder="EUR" />
                                    </div>
                                </div>
                                <div className="grid gap-2">
                                    <Label htmlFor="beschreibung">Beschreibung</Label>
                                    <Textarea id="beschreibung" value={beschreibung} onChange={(e) => setBeschreibung(e.target.value)} placeholder="Optional" />
                                </div>
                            </div>
                            <DialogFooter>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Bestehende Beitragssätze bearbeiten oder löschen.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <Table>
                            <TableHeader>
                                ...
                                {beitragssaetze.map((satz) => (
                                    <TableRow key={satz._id}>
                                        ...
                                        <TableCell className="space-x-2">
                                            <Button variant="outline" size="sm" onClick={() => openEdit(satz._id)}>
                                                Bearbeiten
                                            </Button>
                                            <Button variant="destructive" size="sm" onClick={() => handleDelete(satz._id)}>
                                                Löschen
                                            </Button>
                                        </TableCell>
                                    </TableRow>
                ...
                {editBeitragssatz && isDialogOpen ? <div className="sr-only">Bearbeite {editBeitragssatz.name}</div> : null}
            </div>
        </>
    );
}
