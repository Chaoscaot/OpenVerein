...
import { berechnePositionssumme, decodeDokumentKommentar, DokumentPosition, encodeDokumentKommentar, FinanzDokumentTyp, generateZugferdEmbeddedPdf } from "@/lib/finance-documents";
...
export default function RechnungenPage() {
    ...
    const [isRechnungHinzufuegenOpen, setIsRechnungHinzufuegenOpen] = useState(false);
    const [isRechnungErstellenOpen, setIsRechnungErstellenOpen] = useState(false);
    const [editingRechnungId, setEditingRechnungId] = useState<Id<"buchung_rechnung"> | undefined>(undefined);
    const [rechnungDatei, setRechnungDatei] = useState<File | null>(null);
    const [rechnungFormat, setRechnungFormat] = useState<"pdf" | "e-rechnung">("pdf");
    ...
    const [rechnungDatum, setRechnungDatum] = useState<Date | undefined>(new Date());
    const [rechnungEmpfaenger, setRechnungEmpfaenger] = useState("");
    const [rechnungKommentar, setRechnungKommentar] = useState("");
    const [itemisierung, setItemisierung] = useState<Array<{ id: string; bezeichnung: string; menge: string; einzelpreis: string }>>([]);

    const rechnungenByBuchung = useMemo(() => {
        const map = new Map<Id<"kassen_buchung">, (typeof rechnungen extends undefined ? never : NonNullable<typeof rechnungen>[number])[]>();
    ...
    const selectedBuchung = buchungen?.find((buchung) => buchung._id === buchungId);
    ...
    const isRechnungDateiGueltig = (file: File, format: "pdf" | "e-rechnung") => {
        ...
        return file.type === "application/pdf" || file.type === "application/xml" || file.type === "text/xml" || lowerName.endsWith(".pdf") || lowerName.endsWith(".xml");
    };
    ...
    const handleEditRechnungDialog = (rechnung: NonNullable<typeof rechnungen>[number]) => {
        ...
        setRechnungDatum(rechnung.rechnungsdatum ? new Date(rechnung.rechnungsdatum) : undefined);
    ...
    const parseItemisierung = (): { hasAny: boolean; positionen: DokumentPosition[]; summe: number } | null => {
        const rows = itemisierung.filter((item) => item.bezeichnung.trim() || item.menge.trim() || item.einzelpreis.trim());
        if (rows.length === 0) {
    ...
    const handleDownloadRechnungDatei = async (fileId: string, filename: string) => {
        try {
            const url = await convex.query(api.files.getUrl, { fileId });
    ...
    const handleSaveRechnung = async () => {
        ...
        const parsedItemisierung = dokumentTyp === "rechnung" ? parseItemisierung() : { hasAny: false, positionen: [], summe: 0 };
        ...
        if (rechnungDatei && !isRechnungDateiGueltig(rechnungDatei, rechnungFormat)) {
            toast.error(rechnungFormat === "pdf" ? "Für PDF nur PDF-Dateien hochladen" : "Für E-Rechnung bitte PDF- oder XML-Datei hochladen");
            return;
        }
    ...
    const handleGenerateDokumente = async () => {
        ...
        const mitgliedName = selectedBuchung.mitgliedId
            ? (() => {
                  const mitglied = (mitglieder ?? []).find((m) => m._id === selectedBuchung.mitgliedId);
                  return mitglied ? `${mitglied.vorname} ${mitglied.nachname}` : undefined;
              })()
            : undefined;
        ...
        try {
            ...
            const pdfFile = new File([pdfPart], `${baseName}.pdf`, { type: "application/pdf" });
    ...
    if (kasse === undefined || buchungen === undefined || rechnungen === undefined || mitglieder === undefined || verein === undefined) {
        ...
        <>
            ...
            <div className="space-y-6 p-4">
                <div className="flex items-center gap-4">
                    <Button variant="outline" size="sm" onClick={() => router.push(`/verein/${vereinId}/finanzen/${kasseId}`)}>
                        &larr; Zurück zur Kasse
                    </Button>
                    <div>
                        <h2 className="text-3xl font-bold tracking-tight">Rechnungsmanagement</h2>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Rechnungen werden nur für die ausgewählte Buchung angezeigt.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        {!selectedBuchung && <p className="text-sm text-muted-foreground">Die angegebene Buchung wurde nicht gefunden.</p>}
                        {selectedBuchung && (
                            <div className="space-y-4">
                                <div className="rounded-md border p-3 space-y-1">
                                    <p className="text-sm text-muted-foreground">Datum</p>
                                    <p className="font-medium">{format(new Date(selectedBuchung.datum), "dd.MM.yyyy")}</p>
                                    ...
                                    <p className={`font-medium ${selectedBuchung.betrag < 0 ? "text-red-500" : "text-green-500"}`}>
                                ...
                                <div className="flex justify-end">
                                    <div className="flex gap-2">
                                        <Button variant="outline" size="sm" onClick={handleCreateDokumentDialog}>
                                            Rechnung erstellen
                                        </Button>
                                        <Button variant="outline" size="sm" onClick={handleCreateRechnungDialog}>
                                            Rechnung hinzufügen
                                        </Button>
                                    </div>
                                </div>
                ...
                {selectedBuchung && (
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Es werden nur Rechnungen dieser Buchung angezeigt.</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-2">
                            {selectedBuchungRechnungen.length === 0 && <p className="text-sm text-muted-foreground">Noch keine Rechnungen hochgeladen.</p>}
                            {selectedBuchungRechnungen.map((rechnung) => (
                                <div key={rechnung._id} className="flex items-center justify-between gap-2 text-sm rounded-md border p-3">
                                    <div>
                                        <p className="font-medium">{rechnung.dateiname}</p>
                                        <p className="text-muted-foreground">{decodeDokumentKommentar(rechnung.kommentar).dokumentTyp === "spendenquittung" ? "Spendenquittung" : "Rechnung"}</p>
                                        <p className={rechnung.betragAbgleich ? "text-green-600" : "text-red-500"}>
                                            {rechnung.betrag.toFixed(2)} {rechnung.waehrung} · {rechnung.betragAbgleich ? "Betrag passt" : "Betrag weicht ab"}
                                        </p>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <Button variant="outline" size="sm" onClick={() => handleOpenRechnungDatei(rechnung.fileId)}>
                                            Öffnen
                                        </Button>
                                        <Button variant="outline" size="sm" onClick={() => handleEditRechnungDialog(rechnung)}>
                                            Bearbeiten
                                        </Button>
                                        <Button variant="outline" size="sm" onClick={() => handleDownloadRechnungDatei(rechnung.fileId, rechnung.dateiname)}>
                                            Download
                                        </Button>
                                        <Button variant="destructive" size="sm" onClick={() => handleDeleteRechnung(rechnung._id)}>
                                            Löschen
                                        </Button>
                                    </div>
                                </div>
                ...
                <Dialog open={isRechnungHinzufuegenOpen} onOpenChange={setIsRechnungHinzufuegenOpen}>
                    <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
                        <DialogHeader>
                            <DialogTitle>{editingRechnungId ? "Rechnung bearbeiten" : "Rechnung hinzufügen"}</DialogTitle>
                            <DialogDescription>
                                Buchung: {selectedBuchung?.zweck || "-"} · Verbucht: {selectedBuchung?.betrag.toFixed(2)} {kasse.waehrung}
                            </DialogDescription>
                        </DialogHeader>
                        ...
                        {selectedBuchung && (
                            <div className="rounded-md border p-3 space-y-2">
                                ...
                                {selectedBuchungRechnungen.length === 0 && <p className="text-sm text-muted-foreground">Noch keine Rechnungen hochgeladen.</p>}
                                {selectedBuchungRechnungen.map((rechnung) => (
                                    <div key={rechnung._id} className="flex items-center justify-between gap-2 text-sm">
                                        <div>
                                            <p className="font-medium">{rechnung.dateiname}</p>
                                            <p className="text-muted-foreground">{decodeDokumentKommentar(rechnung.kommentar).dokumentTyp === "spendenquittung" ? "Spendenquittung" : "Rechnung"}</p>
                                            <p className={rechnung.betragAbgleich ? "text-green-600" : "text-red-500"}>
                        ...
                        <div className="grid gap-4 py-2">
                            <div className="grid gap-2">
                                ...
                                <Select value={dokumentTyp} onValueChange={(val: FinanzDokumentTyp) => setDokumentTyp(val)}>
                                    <SelectTrigger id="dokumentTyp">
                                        <SelectValue placeholder="Dokumenttyp wählen" />
                                    </SelectTrigger>
                                    <SelectContent>
                                        <SelectItem value="rechnung">Rechnung</SelectItem>
                                        <SelectItem value="spendenquittung">Spendenquittung</SelectItem>
                                    </SelectContent>
                                </Select>
                            </div>
                            <div className="grid gap-2">
                                <Label htmlFor="rechnungFormat">Format</Label>
                                <Select value={rechnungFormat} onValueChange={(val: "pdf" | "e-rechnung") => setRechnungFormat(val)}>
                            ...
                            <div className="grid gap-2">
                                <Label htmlFor="rechnungDatei">Rechnungsdatei {editingRechnungId ? "(optional für Ersetzen)" : ""}</Label>
                                <Input
                                    id="rechnungDatei"
                                    type="file"
                                    accept={rechnungFormat === "pdf" ? ".pdf,application/pdf" : ".pdf,.xml,application/pdf,application/xml,text/xml"}
                                    onChange={(e) => setRechnungDatei(e.target.files?.[0] ?? null)}
                                />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div className="grid gap-2">
                                    <Label>Rechnungsbetrag</Label>
                                    <Input value={`${Math.abs(selectedBuchung?.betrag ?? 0).toFixed(2)}`} readOnly />
                                </div>
                                <div className="grid gap-2">
                                    <Label htmlFor="rechnungWaehrung">Währung</Label>
                                    <Input id="rechnungWaehrung" value={rechnungWaehrung} onChange={(e) => setRechnungWaehrung(e.target.value.toUpperCase())} placeholder="EUR" />
                                </div>
                            </div>
                                ...
                                <div className="space-y-2">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <Button
                                            ...
                                            variant="outline"
                                            size="sm"
                                            onClick={() => setItemisierung((prev) => [...prev, { id: `${Date.now()}-${Math.random()}`, bezeichnung: "", menge: "1", einzelpreis: "0" }])}
                                        >
                                            + Position
                                        </Button>
                                    </div>
                                    {itemisierung.length === 0 && <p className="text-sm text-muted-foreground">Keine Positionen hinterlegt.</p>}
                                        ...
                                        <div key={item.id} className="grid grid-cols-12 gap-2 items-end">
                                            <div className="col-span-6 grid gap-1">
                                                ...
                                                <Input
                                                    ...
                                                    onChange={(e) => setItemisierung((prev) => prev.map((row) => (row.id === item.id ? { ...row, bezeichnung: e.target.value } : row)))}
                                                />
                                            </div>
                                            <div className="col-span-2 grid gap-1">
                                                ...
                                                <Input
                                                    ...
                                                    step="0.01"
                                                    value={item.menge}
                                                    onChange={(e) => setItemisierung((prev) => prev.map((row) => (row.id === item.id ? { ...row, menge: e.target.value } : row)))}
                                                />
                                            </div>
                                            <div className="col-span-3 grid gap-1">
                                                ...
                                                <Input
                                                    ...
                                                    onChange={(e) => setItemisierung((prev) => prev.map((row) => (row.id === item.id ? { ...row, einzelpreis: e.target.value } : row)))}
                                                />
                                            </div>
                                            <div className="col-span-1">
                                                <Button type="button" variant="destructive" size="sm" onClick={() => setItemisierung((prev) => prev.filter((row) => row.id !== item.id))}>
                                                    -
                                                </Button>
                                            </div>
                                        </div>
                                    ))}
                                    {typeof itemisierungsSumme === "number" && itemisierung.length > 0 && (
                                        <p className="text-sm text-muted-foreground">
                                            Positionssumme: {itemisierungsSumme.toFixed(2)} {rechnungWaehrung} · Gebucht: {Math.abs(selectedBuchung?.betrag ?? 0).toFixed(2)} {rechnungWaehrung}
                                        </p>
                                    )}
                                </div>
                            )}
                            <div className="grid grid-cols-2 gap-4">
                                <div className="grid gap-2">
                                    ...
                                    <Input id="rechnungNummer" value={rechnungNummer} onChange={(e) => setRechnungNummer(e.target.value)} placeholder="Optional" />
                                </div>
                                <div className="grid gap-2 flex-col flex">
                                    <Label htmlFor="rechnungDatum">Rechnungsdatum</Label>
                                    <Popover>
                                        <PopoverTrigger asChild>
                                            <Button variant={"outline"} className={cn("w-full justify-start text-left font-normal", !rechnungDatum && "text-muted-foreground")}>
                                                {rechnungDatum ? format(rechnungDatum, "PPP", { locale: de }) : <span>Datum wählen</span>}
                                            </Button>
                                        </PopoverTrigger>
                                        <PopoverContent className="w-auto p-0">
                                            <Calendar mode="single" selected={rechnungDatum} onSelect={setRechnungDatum} initialFocus />
                                        </PopoverContent>
                                    </Popover>
                                </div>
                            </div>
                            <div className="grid gap-2">
                                ...
                                <Input id="rechnungEmpfaenger" value={rechnungEmpfaenger} onChange={(e) => setRechnungEmpfaenger(e.target.value)} placeholder="Optional" />
                            </div>
                            <div className="grid gap-2">
                                <Label htmlFor="rechnungKommentar">Kommentar</Label>
                                <Input id="rechnungKommentar" value={rechnungKommentar} onChange={(e) => setRechnungKommentar(e.target.value)} placeholder="Optional" />
                            </div>
                        </div>
                        ...
                        <DialogFooter>
                            ...
                            <Button onClick={handleSaveRechnung}>{editingRechnungId ? "Rechnung aktualisieren" : "Rechnung speichern"}</Button>
                        </DialogFooter>
                    </DialogContent>
                </Dialog>

                <Dialog open={isRechnungErstellenOpen} onOpenChange={setIsRechnungErstellenOpen}>
                    <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
                        <DialogHeader>
                        ...
                        <div className="grid gap-4 py-2">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="grid gap-2">
                                    ...
                                    <Input id="rechnungWaehrungErstellen" value={rechnungWaehrung} onChange={(e) => setRechnungWaehrung(e.target.value.toUpperCase())} placeholder="EUR" />
                                </div>
                            </div>
                            {dokumentTyp === "rechnung" && (
                                <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="grid gap-2">
                                    <Label htmlFor="rechnungNummerErstellen">Rechnungsnummer</Label>
                                    <Input id="rechnungNummerErstellen" value={rechnungNummer} onChange={(e) => setRechnungNummer(e.target.value)} placeholder="Optional" />
                                </div>
                                <div className="grid gap-2 flex-col flex">
                            ...
                            <div className="grid gap-2">
                                <Label htmlFor="rechnungEmpfaengerErstellen">Rechnungsempfänger</Label>
                                <Input id="rechnungEmpfaengerErstellen" value={rechnungEmpfaenger} onChange={(e) => setRechnungEmpfaenger(e.target.value)} placeholder="Optional" />
                            </div>
                            <div className="grid gap-2">
                                <Label htmlFor="rechnungKommentarErstellen">Kommentar</Label>
                                <Input id="rechnungKommentarErstellen" value={rechnungKommentar} onChange={(e) => setRechnungKommentar(e.target.value)} placeholder="Optional" />
                            </div>
                        </div>
                        ...
                        <DialogFooter>
                            ...
                            <Button onClick={handleGenerateDokumente}>PDF + ZUGFeRD erzeugen</Button>
                        </DialogFooter>
                    </DialogContent>
                </Dialog>
            </div>
