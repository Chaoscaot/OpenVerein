...
export function encodeDokumentKommentar(
...
export function decodeDokumentKommentar(rawKommentar?: string) {
  ...
  const dokumentTyp: FinanzDokumentTyp =
    rawType === "spendenquittung" ? "spendenquittung" : "rechnung";
...
export async function generateDokumentPdf(params: {
  ...
  const empfaengerName =
    params.rechnungsempfaenger?.trim() || params.mitgliedName?.trim() || "-";
  ...
  // =============================================
  let y = pageHeight - 50;
  page.drawText(params.vereinName, {
    x: ml,
    y,
    size: 18,
    font: fontBold,
  ...
  y -= 16;
  page.drawText(params.vereinAddress, {
    x: ml,
    y,
    size: 9,
    font,
  ...
  // =============================================
  const title =
    params.dokumentTyp === "rechnung" ? "Rechnung" : "Spendenquittung";
  ...
  y -= 32;
  const hasPositionen =
    params.dokumentTyp === "rechnung" && (params.positionen?.length ?? 0) > 0;
  ...
  if (hasPositionen) {
    ...
    for (let i = 0; i < (params.positionen?.length ?? 0); i++) {
      ...
      drawTextRight(
        `${pos.einzelpreis.toFixed(2)} ${params.waehrung}`,
        colEinzelR,
        y + 1,
        9,
        font,
      ...
      drawTextRight(
        `${lineTotal.toFixed(2)} ${params.waehrung}`,
        colSummeR,
        y + 1,
        9,
        font,
    ...
    page.drawLine({
      start: { x: ml, y: y + rowH - 6 },
      end: { x: rightEdge, y: y + rowH - 6 },
      thickness: 0.6,
      color: borderGray,
    });
  } else {
  ...
  if (params.dokumentTyp === "spendenquittung") {
    ...
    page.drawText("Diese Spendenquittung wurde elektronisch erstellt.", {
      x: ml + 10,
      y: y + 1,
      size: 9,
      font,
  ...
  page.drawLine({
    start: { x: ml, y: 52 },
    end: { x: rightEdge, y: 52 },
    thickness: 0.8,
    color: accentColor,
  });
  page.drawText("Automatisch erstellt durch OpenVerein", {
    x: ml,
    y: 38,
    size: 8,
    font,
...
export function generateZugferdXml(params: {
  ...
  const issueDate = (params.dokumentDatumIso ?? new Date().toISOString())
    .slice(0, 10)
    .replaceAll("-", "");
  const bookingDate = params.buchungsDatumIso.slice(0, 10).replaceAll("-", "");
  const empfaengerName =
    params.rechnungsempfaenger?.trim() ||
    params.mitgliedName?.trim() ||
    "Unbekannt";
  const guideline = "urn:cen.eu:en16931:2017";
  const idPrefix = params.dokumentTyp === "rechnung" ? "INV" : "DON";
  const dokumentId = escapeXml(
    params.dokumentNummer || `${idPrefix}-${Date.now()}`,
  );
